...
- **A tela de Cobrança automática deixou de mostrar um quadro que repetia o que o cartão de situação da conta já dizia, e saiu também a linha sobre conta bancária desse cartão.** Aquela linha tratava como pendência sua algo que não é: a conta bancária para onde o dinheiro do condomínio é transferido depois de recebido é assunto que se resolve direto no painel do Asaas, e não é o V3RCondo quem decide ou informa esse caminho. Em compensação, o cartão de situação da conta passou a mostrar **desde quando** o cadastro está com o Asaas, para você acompanhar se a análise está demorando.
...
- **Cada versão da ata passou a ter seu próprio arquivo.** Quando você pedia uma nova ata para a mesma assembleia, a versão nova **gravava por cima** da anterior. Agora cada geração fica guardada separadamente, e o nome do arquivo que você baixa continua o mesmo de sempre, legível — sem códigos no meio. Vale também para o **edital**.
- **A regra de "uma ata por assembleia" agora vale também quando você lança a sua própria ata.** Se preferir escrever a ata por fora e **enviar o documento pronto**, as versões geradas antes saem da lista automaticamente, como já acontecia quando a ata era gerada pelo sistema. E o documento sai das **duas** listas onde aparecia — nos documentos da assembleia **e** no módulo Documentos.
- **A ata agora sai com a logo do seu condomínio.** O cabeçalho do documento passou a trazer a **logo do condomínio** (cadastrada em Configurações), com a marca V3RCondo no rodapé — o mesmo padrão dos demais documentos emitidos pela plataforma. Se o condomínio ainda não tiver logo cadastrada, a do V3RCondo é usada no lugar. A imagem é exibida na proporção original, sem distorção.
- **Excluir um condômino ficou mais claro — e não apaga o histórico.** Antes, ao excluir um condômino que já tinha participado de uma assembleia, a operação **falhava** com uma mensagem técnica em inglês. Agora funciona, e o significado ficou explícito: **excluir = a pessoa sai da lista de condôminos, mas o histórico dela permanece** (presenças em assembleia, lançamentos, solicitações). Se ela voltar ao condomínio depois — pela inclusão direta ou pela importação por planilha —, volta no mesmo cadastro, sem duplicar. **Desativar continua sendo a pausa:** o condômino segue aparecendo na lista, marcado como inativo.
  - **Autorização de visitantes:** quem foi excluído (ou desativado) do condomínio **deixa de conseguir autorizar a entrada de visitantes**. Antes isso ainda era possível.
- **Últimos ajustes de fuso horário.** Completando o trabalho da versão anterior, mais três pontos passaram a respeitar o fuso do condomínio: a **data da próxima cobrança da assinatura**, a **confirmação de exclusão de conta** e o **nome do mês** na prestação de contas e no relatório de gestão.
...
- **Prestação de contas mais enxuta:** ao anexar comprovantes ao relatório de prestação de contas, a primeira página de cada comprovante agora aparece **logo abaixo da descrição do lançamento, na mesma folha** — acabou aquela página quase em branco antes de cada anexo. Comprovantes com várias páginas continuam completos (as páginas seguintes em tamanho normal).
